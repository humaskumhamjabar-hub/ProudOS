    ...
    <div class="mx-auto max-w-[1500px] space-y-6 px-4 py-6 sm:px-6 lg:px-8">
        <header class="relative overflow-hidden rounded-2xl border border-zinc-200 bg-white px-6 py-6 text-zinc-950 shadow-sm dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-100 sm:px-8">
            <div class="relative grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end">
                <div>
                    <p class="text-xs font-semibold uppercase tracking-[.16em] text-indigo-600 dark:text-indigo-400">People operations</p>
                    <h1>Kelola Tim</h1>
                    <p class="mt-3 max-w-2xl text-sm leading-6 text-zinc-500 dark:text-zinc-400">Satu meja untuk akun, peran, masa akses, batch magang, dan ketidakhadiran. Semua perubahan tetap meninggalkan jejak.</p>
                </div>
                <div class="grid grid-cols-2 gap-3">
                    <div class="rounded-xl border border-zinc-200 bg-zinc-50 px-5 py-4 text-center dark:border-zinc-700 dark:bg-zinc-950">
                        <strong class="block text-2xl font-semibold">{{ $totalAktif }}</strong>
                        <span class="text-[.6rem] font-semibold uppercase tracking-[.14em] text-zinc-500">Anggota aktif</span>
                    </div>
                    <div class="rounded-xl border border-amber-200 bg-amber-50 px-5 py-4 text-center dark:border-amber-900 dark:bg-amber-950/40">
                        <strong class="block text-2xl font-semibold text-amber-700 dark:text-amber-300">{{ $aksesAkanHabis }}</strong>
                        <span class="text-[.6rem] font-semibold uppercase tracking-[.14em] text-amber-700 dark:text-amber-300">Akses ≤ 30 hari</span>
                    </div>
                </div>
            </div>
        </header>
        ...
        <section class="grid gap-3 rounded-3xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-900 md:grid-cols-2 xl:grid-cols-[1.4fr_.8fr_.8fr_auto] xl:items-end">
            ...
            <div class="flex gap-2">
                <button wire:click="buatBatch" class="rounded-lg border border-zinc-300 bg-white px-4 py-2.5 text-xs font-semibold text-zinc-950 hover:bg-zinc-100 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100 dark:hover:bg-zinc-800">+ Batch</button>
                <button wire:click="buatAnggota" class="rounded-lg bg-indigo-600 px-5 py-2.5 text-xs font-semibold text-white hover:bg-indigo-700">+ Anggota</button>
            </div>
        </section>

        @if ($formAnggotaTerbuka)
            <section class="rounded-[2rem] border border-zinc-200 bg-white p-5 shadow-xl dark:border-zinc-800 dark:bg-zinc-900 sm:p-7">
                <div class="flex items-center justify-between gap-4">
                    <div><p class="text-[.65rem] font-semibold uppercase tracking-[.18em] text-indigo-600 dark:text-indigo-400">Profil & akses</p><h2 class="mt-1 text-3xl font-semibold">{{ $anggotaId ? 'Ubah anggota' : 'Anggota baru' }}</h2></div>
                    <button wire:click="tutupForm" class="rounded-full border border-zinc-300 px-3 py-1.5 text-xs font-semibold dark:border-zinc-700">Tutup</button>
                </div>
                ...
                <form wire:submit="simpanAnggota" class="mt-6 grid gap-4 lg:grid-cols-3">
                    ...
                    <div class="lg:col-span-3"><button class="rounded-xl bg-indigo-600 px-6 py-3 text-sm font-semibold text-white hover:bg-indigo-700">Simpan anggota</button></div>
                </form>
            </section>
        @endif

        @if ($formBatchTerbuka)
            <section class="rounded-[2rem] border border-zinc-200 bg-white p-6 shadow-xl dark:border-zinc-800 dark:bg-zinc-900">
                <div class="flex items-center justify-between"><h2 class="text-2xl font-semibold">Batch magang baru</h2><button wire:click="tutupForm" class="text-xs font-semibold">Tutup</button></div>
                <form wire:submit="simpanBatch" class="mt-4 grid gap-4 md:grid-cols-[1.5fr_1fr_1fr_auto] md:items-end">
                    ...
                    <button class="rounded-xl bg-indigo-600 px-5 py-2.5 text-xs font-semibold text-white hover:bg-indigo-700">Simpan batch</button>
                </form>
            </section>
        @endif
        ...
        @if ($formKetidakhadiranTerbuka)
            <section class="rounded-[2rem] border border-zinc-200 bg-white p-6 shadow-xl dark:border-zinc-800 dark:bg-zinc-900">
                <div class="flex items-center justify-between"><div><p class="text-[.65rem] font-semibold uppercase tracking-[.18em] text-indigo-600 dark:text-indigo-400">Blokir ketersediaan</p><h2 class="text-2xl font-semibold">Catat ketidakhadiran</h2></div><button wire:click="tutupForm" class="text-xs font-semibold">Tutup</button></div>
                <form wire:submit="simpanKetidakhadiran" class="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-[.7fr_1fr_1fr_1.4fr_auto] lg:items-end">
                    ...
                    <button class="rounded-xl bg-indigo-600 px-5 py-2.5 text-xs font-semibold text-white hover:bg-indigo-700">Simpan</button>
                </form>
            </section>
        @endif
        ...
        <section class="grid gap-4 lg:grid-cols-2">
            @forelse ($anggota as $item)
                ...
                <article wire:key="anggota-{{ $item->id }}" class="rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg dark:border-zinc-800 dark:bg-zinc-900">
                    <div class="flex items-start justify-between gap-4">
                        <div class="flex min-w-0 gap-3">
                            <div class="flex size-12 shrink-0 items-center justify-center rounded-2xl bg-indigo-600 text-lg font-semibold text-white">{{ $item->initials() }}</div>
                    ...
                    <div class="mt-4"><button wire:click="catatKetidakhadiran({{ $item->id }})" class="text-xs font-semibold text-indigo-700 hover:text-indigo-600 dark:text-indigo-300">+ Catat cuti / izin / sakit</button></div>
                </article>
            @empty
